...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return constructMetadata({
    title: { bn: "সহকর্মীবৃন্দ | মূল্যায়ন অ্যাপ", en: "Department Colleagues | Mulyayon App" },
    description: { bn: "কাজ হোক প্রমাণ", en: "Let Your Work Speak" },
    path: "/teacher/colleagues",
    locale,
  });
}

export default async function ColleaguesPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const locale = lang === "bn" ? "bn" : "en";
